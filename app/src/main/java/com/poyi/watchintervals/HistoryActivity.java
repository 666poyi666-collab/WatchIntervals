...
public class HistoryActivity extends Activity {
    ...
    private void showList() {
        ...
        TextView summary = Ui.text(this, records.isEmpty() ? "还没有训练记录" : records.size() + " 次训练", 12, Ui.MUTED);
        summary.setGravity(Gravity.CENTER);
        root.addView(summary, new LinearLayout.LayoutParams(-1, Ui.dp(this, 30)));
        TextView pages = Ui.text(this, "○  ●  ○   右滑进入训练计划", 11, Ui.MUTED);
        pages.setGravity(Gravity.CENTER); root.addView(pages, new LinearLayout.LayoutParams(-1, Ui.dp(this, 24)));
    ...
    private View recordRow(WorkoutRecord record) {
        ...
        TextView date = Ui.bold(this, new SimpleDateFormat("MM月dd日  HH:mm", Locale.CHINA).format(new Date(record.startedAt)), 15, Ui.WHITE);
        TextView data = Ui.text(this, formatDistance(record.distanceMeters) + "  ·  " + formatDuration(record.durationMs) + "  ·  " + record.steps + " 步", 12, Ui.MUTED);
        row.addView(date, new LinearLayout.LayoutParams(-1, Ui.dp(this, 25)));
        row.addView(data, new LinearLayout.LayoutParams(-1, Ui.dp(this, 24)));
    ...
    private void showDetail(WorkoutRecord record) {
        ...
        addMetric(metrics, "距离", formatDistance(record.distanceMeters));
        addMetric(metrics, "用时", formatDuration(record.durationMs));
        addMetric(metrics, "步数", record.steps + "");
        ...
        if(record.steps>0&&record.durationMs>=30_000)addMetric(metrics2,"平均步频",Math.round(record.steps*60_000d/record.durationMs)+" spm");
    ...
    private View header(String titleText, Runnable backAction) {
        ...
        TextView back = Ui.action(this, "‹", 30, Ui.WHITE, Ui.BLACK); back.setOnClickListener(v -> backAction.run());
        TextView title = Ui.bold(this, titleText, 21, Ui.WHITE);
        header.addView(back, new LinearLayout.LayoutParams(Ui.dp(this, 42), Ui.dp(this, 42)));
    ...
    private void addMetric(LinearLayout row, String label, String value) {
        ...
        TextView data = Ui.bold(this, value, 16, Ui.WHITE); data.setGravity(Gravity.CENTER);
