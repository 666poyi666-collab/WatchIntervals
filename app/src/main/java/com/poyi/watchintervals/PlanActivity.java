...
public class PlanActivity extends Activity {
    ...
    private void buildUi() {
        ...
        TextView title=Ui.bold(this,"选择训练安排",19,Ui.WHITE);identity.addView(title,new LinearLayout.LayoutParams(-1,Ui.dp(this,24)));
        TextView source=Ui.text(this,"离线可用 · 手机端管理",Ui.CAPTION,Ui.MUTED);identity.addView(source,new LinearLayout.LayoutParams(-1,Ui.dp(this,14)));
        nav.addView(identity,new LinearLayout.LayoutParams(0,Ui.dp(this,40),1));root.addView(nav,new LinearLayout.LayoutParams(-1,Ui.dp(this,42)));
        ...
        TextView previewLabel=Ui.bold(this,"安排详情",16,Ui.WHITE);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,Ui.dp(this,32));lp.topMargin=Ui.dp(this,8);root.addView(previewLabel,lp);
        LinearLayout preview=Ui.card(this);
        LinearLayout previewHead=new LinearLayout(this);previewHead.setGravity(Gravity.CENTER_VERTICAL);
        previewHead.addView(Ui.workoutGlyph(this,Ui.LIME),new LinearLayout.LayoutParams(Ui.dp(this,38),Ui.dp(this,38)));
        LinearLayout previewIdentity=new LinearLayout(this);previewIdentity.setOrientation(LinearLayout.VERTICAL);
        previewName=Ui.bold(this,"",19,Ui.WHITE);previewName.setSingleLine(false);previewName.setMaxLines(2);previewIdentity.addView(previewName,new LinearLayout.LayoutParams(-1,-2));
        previewMeta=Ui.text(this,"",Ui.CAPTION,Ui.LIME);previewIdentity.addView(previewMeta,new LinearLayout.LayoutParams(-1,Ui.dp(this,20)));
        LinearLayout.LayoutParams previewIdentityParams=new LinearLayout.LayoutParams(0,-2,1);previewIdentityParams.leftMargin=Ui.dp(this,9);previewHead.addView(previewIdentity,previewIdentityParams);preview.addView(previewHead,new LinearLayout.LayoutParams(-1,-2));
        ...
        TextView select=Ui.action(this,"使用这个安排",17,Ui.BLACK,Ui.LIME);LinearLayout.LayoutParams selectLp=new LinearLayout.LayoutParams(-1,Ui.dp(this,48));selectLp.topMargin=Ui.dp(this,9);root.addView(select,selectLp);
        ...
        confirmationPanel=buildConfirmation();FrameLayout.LayoutParams cp=new FrameLayout.LayoutParams(-1,Ui.dp(this,152),Gravity.BOTTOM);cp.leftMargin=Ui.dp(this,12);cp.rightMargin=Ui.dp(this,12);cp.bottomMargin=Ui.dp(this,10);shell.addView(confirmationPanel,cp);
    ...
    private void renderLibrary() {
        ...
        for(int g=0;g<groups.length();g++){
            ...
            TextView heading=Ui.bold(this,group.optString("name")+"  ·  "+matches.size()+" 个安排",14,Ui.CYAN);LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(-1,Ui.dp(this,34));hp.topMargin=Ui.dp(this,4);libraryList.addView(heading,hp);
            ...
            for(JSONObject item:matches){
                boolean selected=selectedId.equals(item.optString("id"));LinearLayout card=new LinearLayout(this);card.setGravity(Gravity.CENTER_VERTICAL);card.setPadding(Ui.dp(this,10),Ui.dp(this,8),Ui.dp(this,10),Ui.dp(this,8));card.setBackground(Ui.outlinedBackground(this,selected?Ui.PANEL_ACTIVE:Ui.PANEL,selected?Ui.LIME:Ui.LINE,18));
                card.addView(Ui.workoutGlyph(this,selected?Ui.LIME:Ui.MUTED),new LinearLayout.LayoutParams(Ui.dp(this,34),Ui.dp(this,34)));
                LinearLayout copy=new LinearLayout(this);copy.setOrientation(LinearLayout.VERTICAL);TextView name=Ui.bold(this,item.optString("name"),17,Ui.WHITE);name.setSingleLine(false);name.setMaxLines(2);copy.addView(name,new LinearLayout.LayoutParams(-1,-2));
                ArrayList<Stage> stages=PlanStore.decode(item.optJSONArray("stages").toString());TextView meta=Ui.text(this,summary(stages),Ui.CAPTION,selected?Ui.LIME:Ui.MUTED);copy.addView(meta,new LinearLayout.LayoutParams(-1,Ui.dp(this,20)));LinearLayout.LayoutParams copyParams=new LinearLayout.LayoutParams(0,-2,1);copyParams.leftMargin=Ui.dp(this,9);card.addView(copy,copyParams);
                if(selected)card.addView(Ui.chip(this,"当前",Ui.LIME,android.graphics.Color.rgb(30,48,14)),new LinearLayout.LayoutParams(Ui.dp(this,54),Ui.dp(this,24)));
                card.setClickable(true);card.setFocusable(true);card.setOnClickListener(v->{previewPlan=item;renderPreview();});LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.bottomMargin=Ui.dp(this,7);libraryList.addView(card,cp);
            }
        }
    }
    ...
    private LinearLayout buildConfirmation(){LinearLayout panel=new LinearLayout(this);panel.setOrientation(LinearLayout.VERTICAL);panel.setPadding(Ui.dp(this,16),Ui.dp(this,12),Ui.dp(this,16),Ui.dp(this,12));panel.setBackground(Ui.background(this,Ui.PANEL_ACTIVE,24));panel.setVisibility(View.GONE);confirmationTitle=Ui.bold(this,"使用这个安排？",19,Ui.WHITE);confirmationHint=Ui.text(this,"",12,Ui.MUTED);panel.addView(confirmationTitle,new LinearLayout.LayoutParams(-1,Ui.dp(this,32)));panel.addView(confirmationHint,new LinearLayout.LayoutParams(-1,Ui.dp(this,26)));LinearLayout buttons=new LinearLayout(this);TextView cancel=Ui.action(this,"取消",15,Ui.WHITE,Ui.PANEL);TextView use=Ui.action(this,"使用",15,Ui.BLACK,Ui.LIME);LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(0,Ui.dp(this,46),1);bp.rightMargin=Ui.dp(this,7);buttons.addView(cancel,bp);buttons.addView(use,new LinearLayout.LayoutParams(0,Ui.dp(this,46),1));panel.addView(buttons);cancel.setOnClickListener(v->hideConfirmation());use.setOnClickListener(v->applySelection());return panel;}
