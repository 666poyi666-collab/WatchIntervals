...
public class PlanActivity extends Activity {
    ...
    private void renderPreview(){if(previewPlan==null)return;previewName.setText(previewPlan.optString("name"));ArrayList<Stage> stages=PlanStore.decode(previewPlan.optJSONArray("stages").toString());previewMeta.setText(PlanLibraryStore.groupName(library,previewPlan.optString("groupId"))+" · "+summary(stages));String req=previewPlan.optString("requirement");previewRequirement.setText(req.isEmpty()?"按下方训练内容顺序完成。":req);previewStages.removeAllViews();for(int i=0;i<stages.size();i++){Stage stage=stages.get(i);TextView row=Ui.text(this,(i+1)+"   "+stage.name()+"   "+stage.targetText(),14,Ui.WHITE);row.setBackground(Ui.background(this,Ui.PANEL_ACTIVE,14));row.setPadding(Ui.dp(this,12),0,Ui.dp(this,12),0);LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(-1,Ui.dp(this,46));rp.topMargin=Ui.dp(this,6);previewStages.addView(row,rp);}}
