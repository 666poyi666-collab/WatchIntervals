...
final class WatchCommandRouter implements AutoCloseable {
    ...
    private Result control(String action,String body)throws Exception{
        ...
        JSONObject cachedEntry=commandCache().optJSONObject(commandId);
        if(cachedEntry!=null){
            JSONObject cachedResult=cachedEntry.optJSONObject("result");
            if(cachedResult==null)return result(new JSONObject(cachedEntry.toString()).put("duplicate",true));
            if(!signature.equals(cachedEntry.optString("signature")))return error(409,"command_id_reused");
            return result(new JSONObject(cachedResult.toString()).put("duplicate",true));
        }
        long expiresAt=command.optLong("expiresAt",Long.MAX_VALUE);
        if(expiresAt<=System.currentTimeMillis())return result(cacheCommand(commandId,signature,
                new JSONObject().put("accepted",false).put("commandId",commandId)
                        .put("error","command_expired").put("httpStatus",409)));
        if("delete_workout".equals(action)){
            ...
            boolean deleted=HistoryStore.delete(context,workoutId);
            return result(cacheCommand(commandId,signature,new JSONObject().put("accepted",true)
                    .put("commandId",commandId).put("action",action).put("deleted",deleted)
                    .put("alreadyAbsent",!deleted)));
        }
        ...
        if(!expected.isEmpty()&&!expected.equals(actual))return result(cacheCommand(commandId,signature,
                new JSONObject().put("accepted",false).put("commandId",commandId)
        ...
        else return error(422,"invalid_action");
        context.startForegroundService(intent);
        return result(cacheCommand(commandId,signature,new JSONObject().put("accepted",true)
                .put("commandId",commandId).put("action",action).put("previousState",actual)));
    }
    ...
    private String commandSignature(String action,JSONObject command){return action+"|"+
    ...
    private JSONObject cacheCommand(String id,String signature,JSONObject result){try{JSONObject cache=commandCache();cache.put(id,new JSONObject().put("signature",signature).put("result",new JSONObject(result.toString())));JSONArray names=cache.names();while(names!=null&&names.length()>100){cache.remove(names.optString(0));names=cache.names();}context.getSharedPreferences("command_cache",Context.MODE_PRIVATE).edit().putString("items",cache.toString()).commit();}catch(Exception ignored){}return result;}
