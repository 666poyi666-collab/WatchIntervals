...
public class HistoryDetailActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        LinearLayout summary=card();summary.addView(text("运动概览",17,true,Color.rgb(24,27,25)));LinearLayout summaryRow=new LinearLayout(this);summaryRow.addView(metricCell("总距离",PhoneFormat.distance(meters),true),weight());summaryRow.addView(metricCell("运动时间",PhoneFormat.duration(duration),false),weight());summaryRow.addView(metricCell("平均配速",PhoneFormat.pace(duration,meters),false),weight());summary.addView(summaryRow);root.addView(summary,marginTop(8));
        ...
        LinearLayout timing=card();timing.addView(text("时间与速度",17,true,Color.rgb(24,27,25)));long elapsed=record.optLong("elapsedDurationMs",duration),paused=record.optLong("pausedDurationMs");timing.addView(dataLine("活动时间",PhoneFormat.duration(duration)));timing.addView(dataLine("总历时",PhoneFormat.duration(elapsed)));timing.addView(dataLine("暂停时间",PhoneFormat.duration(paused)));timing.addView(dataLine("平均时速",meters>0&&duration>0?String.format(Locale.CHINA,"%.1f km/h",meters/duration*3600d):"--"));timing.addView(dataLine("最高平滑时速",record.optDouble("maxSmoothedSpeedMps")>0?String.format(Locale.CHINA,"%.1f km/h",record.optDouble("maxSmoothedSpeedMps")*3.6d):"--"));if(record.optLong("planCompletedActiveMs")>0){timing.addView(dataLine("计划完成",PhoneFormat.duration(record.optLong("planCompletedActiveMs"))));timing.addView(dataLine("自由记录",PhoneFormat.duration(record.optLong("freeRecordingActiveMs"))));}root.addView(timing,marginTop(14));
        JSONObject sources=record.optJSONObject("distanceBySourceMeters");if(sources!=null){LinearLayout sourceCard=card();sourceCard.addView(text("距离来源",17,true,Color.rgb(24,27,25)));String[][] labels={{"system_exercise","系统运动"},{"watch_gps","手表 GPS"},{"phone_gps","手机 GPS"},{"steps_estimate","步数估距"}};for(String[] item:labels){double value=sources.optDouble(item[0]);if(value>0)sourceCard.addView(dataLine(item[1],PhoneFormat.distance(value)));}root.addView(sourceCard,marginTop(14));}
        ...
        JSONArray splits=record.optJSONArray("splits");if(splits!=null&&splits.length()>0){LinearLayout splitCard=card();splitCard.addView(text("公里分段",17,true,Color.rgb(24,27,25)));for(int i=0;i<splits.length();i++){JSONObject split=splits.optJSONObject(i);if(split!=null)splitCard.addView(dataLine(split.optInt("index")+" 公里",PhoneFormat.duration(split.optLong("durationMs"))+"  ·  "+PhoneFormat.paceSeconds(split.optLong("paceSecondsPerKm"))));}root.addView(splitCard,marginTop(14));}
    ...
    private LinearLayout dataLine(String label,String value){LinearLayout row=new LinearLayout(this);row.setPadding(0,dp(9),0,dp(9));TextView left=text(label,14,false,Color.rgb(35,38,36));TextView right=text(value,14,true,Color.rgb(24,27,25));right.setGravity(Gravity.RIGHT);row.addView(left,new LinearLayout.LayoutParams(0,-2,1f));row.addView(right,new LinearLayout.LayoutParams(-2,-2));return row;}
