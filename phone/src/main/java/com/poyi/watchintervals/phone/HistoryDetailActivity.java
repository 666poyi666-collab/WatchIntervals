...
public class HistoryDetailActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        LinearLayout timing=card();timing.addView(text("时间与速度",17,true,Color.rgb(24,27,25)));long elapsed=record.optLong("elapsedDurationMs",duration),paused=record.optLong("pausedDurationMs");timing.addView(dataLine("活动时间",formatDuration(duration)));timing.addView(dataLine("总历时",formatDuration(elapsed)));timing.addView(dataLine("暂停时间",formatDuration(paused)));timing.addView(dataLine("平均时速",meters>0&&duration>0?String.format(Locale.CHINA,"%.1f km/h",meters/duration*3600d):"--"));timing.addView(dataLine("最高平滑时速",record.optDouble("maxSmoothedSpeedMps")>0?String.format(Locale.CHINA,"%.1f km/h",record.optDouble("maxSmoothedSpeedMps")*3.6d):"--"));if(record.optLong("planCompletedActiveMs")>0){timing.addView(dataLine("计划完成",formatDuration(record.optLong("planCompletedActiveMs"))));timing.addView(dataLine("自由记录",formatDuration(record.optLong("freeRecordingActiveMs"))));}root.addView(timing,marginTop(14));
        JSONObject sources=record.optJSONObject("distanceBySourceMeters");if(sources!=null){LinearLayout sourceCard=card();sourceCard.addView(text("距离来源",17,true,Color.rgb(24,27,25)));String[][] labels={{"system_exercise","系统运动"},{"watch_gps","手表 GPS"},{"phone_gps","手机 GPS"},{"steps_estimate","步数估距"}};for(String[] item:labels){double value=sources.optDouble(item[0]);if(value>0)sourceCard.addView(dataLine(item[1],formatDistance(value)));}root.addView(sourceCard,marginTop(14));}
