...
public final class WatchConnectionManager {
    ...
    private WatchConnectionManager(Context context){this.context=context;identity=new WatchIdentityStore(context);ble=new BleGattTransport(context,identity);ble.setStateListener((next,cause)->{ConnectionState previous=state;setState(next,cause);if(next==ConnectionState.DISCONNECTED&&(previous==ConnectionState.CONNECTED_BLE||previous==ConnectionState.CONNECTED_BLE_LAN||previous==ConnectionState.SYNCING))scheduleReconnect();});}
    public WatchIdentityStore identity(){return identity;}
    public void configurePairing(String pairingCode){identity.savePairing(identity.watchDeviceId(),pairingCode);}
    public void configureLan(String host,String pairingCode){lan.configure(host,pairingCode);}
    ...
    public synchronized CompletableFuture<TransportSession> connect(){reconnectScheduled=false;if(identity.pairingCode().length()!=6){setState(ConnectionState.UNPAIRED,"pairing_required");CompletableFuture<TransportSession> result=new CompletableFuture<>();result.completeExceptionally(new IllegalStateException("pairing_required"));return result;}CompletableFuture<TransportSession> result=new CompletableFuture<>();ble.connect().whenComplete((session,error)->{if(error==null){backoff.reset();lastSeen=System.currentTimeMillis();setState(ConnectionState.CONNECTED_BLE,null);result.complete(session);verifyLan();}else if(lan.isAvailable())lan.connect().whenComplete((lanSession,lanError)->{if(lanError==null){lanVerified=true;lastSeen=System.currentTimeMillis();setState(ConnectionState.CONNECTED_LAN,"ble_unavailable");result.complete(lanSession);scheduleReconnect();}else{result.completeExceptionally(lanError);scheduleReconnect();}});else{result.completeExceptionally(error);scheduleReconnect();}});return result;}
    public void connectNow(){backoff.reset();connect();}
    private void verifyLan(){if(!lan.isAvailable()){lanVerified=false;publish();return;}lan.connect().whenComplete((session,error)->{lanVerified=error==null;if(lanVerified)setState(ConnectionState.CONNECTED_BLE_LAN,null);else publish();});}
