...
public final class WatchConnectionManager {
    ...
    public synchronized CompletableFuture<TransportSession> connect(){reconnectScheduled=false;if(!identity.isPaired()&&identity.pairingCode().length()!=6){setState(ConnectionState.UNPAIRED,"pairing_required");CompletableFuture<TransportSession> result=new CompletableFuture<>();result.completeExceptionally(new IllegalStateException("pairing_required"));return result;}CompletableFuture<TransportSession> result=new CompletableFuture<>();if(lan.isAvailable()&&!lanVerified)verifyLan();ble.connect().whenComplete((session,error)->{if(error==null){backoff.reset();lastSeen=System.currentTimeMillis();if(lan.isAvailable())lan.configure(lan.host(),identity.lanCredential());setState(ConnectionState.CONNECTED_BLE,null);result.complete(session);main.post(()->EncryptedWatchSyncWorker.schedule(context));verifyLan();}else if(lan.isAvailable())lan.connect().whenComplete((lanSession,lanError)->{if(lanError==null){lanVerified=true;lastSeen=System.currentTimeMillis();setState(ConnectionState.CONNECTED_LAN,"ble_unavailable");result.complete(lanSession);main.post(()->EncryptedWatchSyncWorker.schedule(context));scheduleReconnect();}else{result.completeExceptionally(lanError);scheduleReconnect();}});else{result.completeExceptionally(error);scheduleReconnect();}});return result;}
