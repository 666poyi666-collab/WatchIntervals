...
final class PhoneSyncPolicy {
    private PhoneSyncPolicy() {}

    static boolean isTransportReady(ConnectionState state) {
    ...
    static boolean shouldAutoSync(ConnectionState previous, ConnectionState current,
            boolean syncInFlight) {
        return !syncInFlight && isTransportReady(current)
                && (previous == null || !isTransportReady(previous));
    }

    static String progressLabel(int completed, int total, String operation) {
    ...
    static String successLabel(boolean hadSleepRecords, String formattedTime) {
