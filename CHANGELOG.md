...
- 手表主页由三屏横向翻页重构为单一纵向信息流：顶栏时钟、本周量、发光开始钮、当前安排块（整块点击进入计划选择）、最近训练块（点击直达该记录详情）与全部历史入口；历史与计划不再各自存在一份滞后的「速览页」副本，每个目的地只保留一个规范界面（REQ-UI-005）。
- 手表横向手势语义收敛：主页右滑退出回表盘、左滑打开历史；历史页右滑返回，不再横向跳转计划页。
