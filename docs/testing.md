...
| WT-023 | 真实沿河轨迹地理细节 | 在 OWW221 打开现有 2.43km、280 点沿河记录，对照河道、桥、堤岸和环线，并检查无网络占位 | 历史原始路线、起终点完整可见且坐标不被改写；底图失败时仍保留轨迹层/加载状态；系统 Baidu 与当前降级地图的视觉差异必须明确记录，用户未确认前不得判定通过 |
