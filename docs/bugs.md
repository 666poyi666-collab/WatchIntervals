...
- 唯一关闭条件：Xiaomi xaga 与 OWW221 同时上线并授权 ADB，先复现一次用户所述偶发断联并保存双端状态，再完成 BLE-004、BLE-005 手表半场、BLE-009、BLE-010；关闭无线 ADB 后核心功能仍可自动恢复。
...
- 验证：Watch 57/57 测试、Lint 0 error 与 debug 构建通过。OWW221 未在线，WT-026/027 是转 `Verified` 的唯一关闭条件。
