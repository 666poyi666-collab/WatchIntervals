...
- 处理：曾尝试卫星细节层，但用户明确否决卫星图，该候选未保留。继续使用高德 GCJ-02 道路瓦片降级并改用语义暗色矩阵。第二次按系统资源核对后，历史地图从 230dp 收为系统同款 164dp；系统包络横向 15dp、纵向 25dp，osmdroid 取统一 25dp，最大 zoom 18、单点 zoom 18；轨迹线为 2.6dp。真实坐标和几何不变。
- 验证：关联 `WT-023`。OWW221 已确认 164dp 紧凑地图、25dp 包络和历史路线显示，详情下方配速/分段前移；但系统运动使用 Baidu Map SDK 7.5.9 与自定义暗色样式，当前页面使用 AMap 道路瓦片降级，provider、坐标转换和道路层级仍不同，本项不能标记 Verified。历史保持原有 2 条。
...
- 根因：旧 schema 的迁移默认值与原始定位 accuracy 没有可追溯区分，不能仅凭 `legacy + 125m` 推断真实采集质量；同时系统运动采用 Baidu 自定义地图，而本应用当前为 AMap 降级，视觉错位不止来自坐标精度。系统旧轨迹实际保存在健康服务 `sport_gps` 表，但 BinderProvider 在 normal permission 之外还执行包签名校验，普通第三方签名不能直接读取。
