...
| REQ-UI-011 | 手机端采用平台中立的 iOS/macOS 启发式功能层 | 内容层保持实色数据卡；仅连接设置与固定底栏使用半透明浮层、细描边和同心圆角；页面使用深色系统栏、实时安全区与 34sp 大标题，滚动内容可从浮层后方完整滚出 | 已实现，API 35 模拟器 1.0×/2.0× 字体与 UI hierarchy 通过；待 PT-026 真实手机确认 |
